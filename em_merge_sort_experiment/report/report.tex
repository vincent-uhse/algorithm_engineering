...
\begin{abstract}
	...
	sizes, with larger block sizes enhancing its efficiency. We also establish that the number of merge rounds scales with an expected \( \mathcal{O}\l(\log(n)\r) \) complexity in relation to the input
	size \( n \), while both algorithms follow a \( \mathcal{O}\l(n\log(n)\r) \) time complexity trend.
...
The parameter \code{block\_size} will allow for inital partitioning of size \code{block\_size} before the External Memory Merge Sort algorithm is executed.
The \code{input\_size} parameter tells how many numbers are contained in the file.
...
\begin{minipage}{0.475\textwidth}
		\begin{tabularx}{\textwidth}{|X|S[table-format=3.1]|S[table-format=3.1]|S[table-format=3.1]|S[table-format=3.1]|}
			\hline
			\multicolumn{1}{|c|}{} & \multicolumn{4}{c|}{Input Size / RAM Size} \\ \hline
			Host & 0.1 & 0.5 & 1 & 10 \\ \hline
			Computer1 & & & & \\ \hline % TODO
			Computer2 & 22.07 & 111.52 & 368.62 & 7828.52 \\ \hline
			Computer3 & & & & \\ \hline % TODO
		\end{tabularx}
		\captionof{table}{Run times in seconds rounded to two decimal places. Algorithm performance comparison on large instances of External Memory Merge Sort versus Classical Merge Sort.}
